...
class AuthManager:
    ...
    async def resolve(self, credentials: SignUpCredentials) -> InterferonUser:
        intf_user: InterferonUser = await self.get_user(credentials)
        if not intf_user.is_default():
            if not intf_user.is_new:
                intf_user = await self.login(credentials)
            return intf_user
        else:
    ...
    async def signup(self, credentials: SignUpCredentials) -> InterferonUser:
        ...
        if not supabase_user.is_default():
            intf_user: InterferonUser = AuthManager.__sb_to_intf_user_conv(
                supabase_user
            )
    ...
    async def login(self, credentials: SignUpCredentials) -> InterferonUser:
        supabase_user: SupabaseUser = await self.login_user.login_with_password(
            credentials
        )
        if not supabase_user.is_default():
            intf_user: InterferonUser = AuthManager.__sb_to_intf_user_conv(
    ...
    async def get_user(self, credentials: SignUpCredentials) -> InterferonUser:
        supabase_user: SupabaseUser
        existed: Existed
        supabase_user, existed = await self.fetch_user.fetch_user(credentials)
