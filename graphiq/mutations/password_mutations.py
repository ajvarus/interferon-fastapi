...
class Mutation:
    ...
    async def store_passwords(
        ...
        if not passwords:
            raise HTTPException(
                status_code=400,
                detail="The list of passwords is empty."
            )
        try:
            query = """#graphql
            ...
            """
            encrypted_passwords: List[PasswordRequest] = await encryptor.encrypt_passwords(passwords=passwords)
            variables = {
                "objects": [
                    {
                        "user_id": ep.user_id,
            ...
            response: APIResponse = await client.rpc("resolve", {
                "query": query,
                "variables": variables
            }).execute()

            if response.data:
                if(inserted_passwords := response.data
                   .get("data").get("insertIntopasswordsCollection").get("records")):
